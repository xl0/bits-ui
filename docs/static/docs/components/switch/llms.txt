...
| `child`             | `Snippet` - type ChildSnippetProps = { checked: boolean; props: Record\<string, unknown>; }; | Use render delegation to render your own element. See [Child Snippet](/docs/child-snippet) docs for more information.`Default:  —— undefined` |         |

| Data Attribute     | Value                             | Description                          | Details |
| ------------------ | --------------------------------- | ------------------------------------ | ------- |
| `data-state`       | `enum` - 'checked' \| 'unchecked' | The switch's checked state.          |         |
...
[Previous Slider](/docs/components/slider) [Next Tabs](/docs/components/tabs)